...
authors' published immigration fit (Application~1, $K=3$) from the
\citet{chen2024sts} replication package, on the corpus their \proglang{R} code
regenerates (\code{parity/sts\_r\_compare.py}). The check is calibrated rather
than absolute: on this small three-topic corpus two independent implementations
partition 341 short documents slightly differently, so the cross-implementation
gap is irreducible. We therefore ask that \pkg{topica}'s \code{STS} sit about as
close to the reference \code{STS} (a topic-word cosine near $0.48$) as the
already-validated \code{STM} sits to \proglang{R} \pkg{stm} on the same corpus,
and that \pkg{topica}'s \code{STS} agree with its own \code{STM}, confirming that
\code{STS} correctly extends \code{STM}. The script reads the (non-redistributable)
