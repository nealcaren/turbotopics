...
can be compared on a single corpus in one session. That same uniformity makes
the framework extensible: a new model that presents the same surface
inherits the entire diagnostic and effect-estimation stack without writing any
of it. \pkg{topica} is built on the premise that the researcher, not the
software, owns the theoretical decisions that make a topic-model study credible:
what the corpus represents, how many topics is the right granularity, what each
topic means. The software supplies the mechanics and the diagnostics to support
those decisions, and declines, for instance, to report confidence intervals for a
model that has no posterior to draw them from. A parallel \proglang{Rust} core
makes the fits fast and reproducible: three to twenty-two times faster than
\proglang{R} \pkg{stm} for the structural and other variational models, at parity
with the compiled \pkg{MALLET} and \pkg{keyATM} samplers, deterministic to the bit
for the variational models and seed-reproducible for the samplers. Each model is
validated against its reference implementation. We describe the design, the
...
implementation, and we report the agreement, including where two correct
...
those decisions and the software supplies the mechanics and the
...
Each of these retains advantages it would be dishonest to ignore, and \pkg{topica}
does not dominate any of them on its own ground. \pkg{gensim}'s online LDA streams a
corpus larger than memory, which \pkg{topica}, holding the document-term matrix in
core, does not. \pkg{BERTopic} ships an integrated embedding-to-topic
pipeline (sentence transformers, class-based TF-IDF representation, and
hierarchical and dynamic topic reduction), whereas \pkg{topica} deliberately takes
vectors the user has already computed and bundles none of that machinery.
\pkg{MALLET} and \pkg{tomotopy} have years of production hardening and, for plain
LDA, an inner loop \pkg{topica} matches rather than beats. And \pkg{stm} is not
merely an implementation but an ecosystem (\pkg{tidystm}, \pkg{stminsights}, and a
decade of applied practice) that a reimplementation has to earn its way into.
\pkg{topica}'s claim is narrower and relational: not that it bests these tools where
they are strong, but that none of them reaches across the others' territory.
...
The narrowness of that boundary is what keeps the unified surface from costing
anything. Each of the more than two dozen models is a native \proglang{Rust}
module (\code{lda.rs}, \code{ctm.rs}, \code{stm.rs}, and the rest) that runs its
own inference loop over the corpus and writes its result into two \proglang{Rust}
\code{ndarray} matrices. When a model's \code{topic\_word} or \code{doc\_topic} is
read, the core materializes that matrix as an ordinary \pkg{NumPy} array; nothing
is recomputed on access, and the \proglang{Python} layer carries no model-specific
code. The diagnostics are then plain \pkg{NumPy} functions that take the two arrays
and never learn which module produced them.

The consequence is that a new model is new \proglang{Rust} on one side of the
boundary and \emph{zero} lines on the other: it inherits the diagnostic, labeling,
and effect-estimation stack by producing the two matrices, not by registering with
anything. The same split lets speed and breadth coexist: inference runs at
native speed across parallel \proglang{Rust} threads, while the interface stays a
thin, \pkg{NumPy}-shaped layer that adds no per-call cost. The extension module
is compiled against the stable \proglang{C} ABI (\pkg{abi3}), so a single binary per
platform serves every supported \proglang{Python} version.
...
varies is the seed. Repeating a fit makes the guarantee visible. The two fits
are equal element for element, not merely close:
...
\end{CodeInput}
\begin{CodeOutput}
topic_word identical: True
doc_topic  identical: True
\end{CodeOutput}
\end{CodeChunk}
...
Behind the breadth is a single statistical skeleton. Every model in \pkg{topica}
factors a corpus into the same two objects: a document-topic matrix $\theta$
(\code{doc\_topic}) giving how much each document draws on each topic, and a
topic-word matrix $\phi$ (\code{topic\_word}) giving which words each topic
favors. What separates the models is not the output but the assumptions that
produce it: how $\theta$ is structured, and where $\phi$ comes from. \code{LDA}
puts independent Dirichlet priors on the rows of $\theta$; \code{CTM} replaces
that prior with a logistic-normal one so topics may correlate; \code{STM} and
\code{DMR} let the $\theta$ prior depend on document covariates, and \code{STM}
further lets $\phi$ vary by covariate through the SAGE mechanism; \code{HDP} drops
the fixed topic count, putting a nonparametric prior on how many columns $\theta$
has; the short-text models tie each document to a single topic; and the
embedding-based models give up the generative story for $\phi$, deriving $\theta$
from a clustering of document vectors and reading $\phi$ off the words each cluster
favors. The models are points in one space (priors and structure on $\theta$, a
generative or a geometric route to $\phi$), not a list of unrelated programs, and
that shared shape is exactly what the contract of Section~\ref{sec:design} exposes.

The family therefore divides, by the route to $\phi$, into two kinds.
...
\subsection{One pattern, many models}

The breadth is not a list of disconnected estimators but the same three steps
applied across the family: construct the model, \code{fit} a corpus, and read the
topics from \code{top\_words}. What changes between rows is the capability the
model adds, not the way it is called. Four models beyond those of the spanning
comparison in Section~\ref{sec:example} (one that infers its own number of
topics, one that conditions topics on document metadata, a short-text model, and
one steered by keywords) run on the same poliblog posts through the identical
pattern:
...
dmr = topica.DMR(num_topics=15, seed=1)                # topics by metadata
dmr.fit(docs, conservative, feature_names=["conservative"])

pt = topica.PT(num_topics=15, num_pseudo=100, seed=1)  # short-text model
...
for name, m in [("HDP", hdp), ("DMR", dmr), ("PT", pt), ("KeyATM", keyatm)]:
    print("%-7s K=%-2d |" % (name, m.num_topics),
          ", ".join(w for w, _ in m.top_words(6)[0]))
\end{CodeInput}
\begin{CodeOutput}
HDP     K=22 | will, american, tax, year, economi, govern
DMR     K=15 | said, report, senat, hous, say, polit
PT      K=15 | will, new, american, year, time, peopl
KeyATM  K=15 | tax, economi, econom, will, govern, oil
\end{CodeOutput}
\end{CodeChunk}

The models differ in what they assume and what they estimate: \code{HDP} chooses
$K=22$ on its own, and \code{KeyATM}'s first topic is the economy theme its seed
words pin (\emph{tax}, \emph{economi}, \emph{oil}). But none of that surfaces at the
call site, and none of it reaches the diagnostics, which read only the matrices
every one of them exposes. Adding the fifth model is the same four lines again.
This listing is reproduced by \code{paper/replication.py}.
...
trade-off. The careful procedure is to fit a small range around a theoretically
plausible $K$, label the topics at each, count the uninterpretable ones, and let
the researcher choose, reporting sensitivity to that choice. The grid makes the
trap concrete: on this corpus coherence is highest at the smallest $K$, which a
metric-maximizing pipeline would seize on, collapsing into five topics the
distinctions a fifteen-topic model draws.

\begin{CodeChunk}
\begin{CodeInput}
for r in topica.search_k(docs, ks=[5, 10, 15, 20], model="lda", iterations=200):
    print("K=%-2d  coherence=%7.2f  exclusivity=%.3f"
          % (r["k"], r["coherence"], r["exclusivity"]))
\end{CodeInput}
\begin{CodeOutput}
K=5   coherence= -44.04  exclusivity=0.660
K=10  coherence= -51.01  exclusivity=0.600
K=15  coherence= -52.39  exclusivity=0.544
K=20  coherence= -56.06  exclusivity=0.560
\end{CodeOutput}
\end{CodeChunk}
...
and says so, rather than returning a number that looks like one. The two calls
are the same line; only the answer differs, and the refusal points to the
estimator that would be valid:

\begin{CodeChunk}
\begin{CodeInput}
topica.standard_errors(stm_model, X=X, feature_names=["conservative"])
topica.standard_errors(bertopic,  X=X, feature_names=["conservative"])
\end{CodeInput}
\begin{CodeOutput}
STM      effect=-0.004  se=0.009  (method of composition)
BERTopic has no posterior over theta for method='composition' (no
logistic-normal or Dirichlet structure). Use method='bootstrap' for
standard errors on this model.
\end{CodeOutput}
\end{CodeChunk}
...
\subsection{Recovering topics we planted}

Agreement with another implementation shows that two engines compute the same
thing; it does not show they compute the \emph{right} thing, since both could be
wrong in the same way. The complementary check fits a corpus whose answer is fixed
in advance. We plant five topics with disjoint vocabularies and draw every document
from a single topic, so a faithful model must recover the five and concentrate each
recovered topic on one planted topic. \emph{Purity} is the probability mass a
fitted topic places on its best-matching planted topic ($1.0$ when a recovered
topic is exactly one true topic), and \emph{recovered} counts how many of the five
are some fitted topic's best match.

\begin{CodeChunk}
\begin{CodeInput}
docs, true_sets = planted_corpus(seed=0)    # five disjoint-vocabulary topics

specs = [("LDA",   topica.LDA(num_topics=5, seed=1),   dict(iterations=400)),
...
print("model  K   purity  recovered")
for name, m, kw in specs:
    m.fit(docs, **kw)                        # HDP is given no K; it infers one
    K, purity, recovered = recovery(m, true_sets)
    print("%-5s  %d   %.3f   %d/5" % (name, K, purity, recovered))
\end{CodeInput}
\begin{CodeOutput}
...
Three inference families told the answer is five (collapsed-Gibbs \code{LDA},
variational \code{CTM}, and the Dirichlet-mixture short-text \code{GSDMM}) recover
the planting almost exactly. The nonparametric \code{HDP}, told nothing about the
number of topics, independently infers five and recovers four of them: a stricter
test it largely, not perfectly, passes, and we report the $0.761$ as readily as the
$1.000$s. Because the planting and the scoring run through the shared contract, the
same loop scores every family (\code{paper/replication.py}).
...
they need not, but whether \pkg{topica} lands on \proglang{R}'s solution as closely as
...
with calibrated uncertainty. Both run on the political-blog corpus of the \pkg{stm}
...
The loop prints one row per model; Table~\ref{tab:spanning} collects the same
numbers. The count-based models resolve
...
the software's. The comparison needed to make that call costs no translation
between tools, because all four models are scored through the identical interface.
...
Scoring is not the only thing the shared surface makes free. The question the
introduction posed, whether a count-based model and a clustering of embeddings
find the \emph{same} themes, is itself a one-call comparison, because
\code{align\_topics} matches topics across any two models by the cosine similarity
of their topic-word distributions:

\begin{CodeChunk}
\begin{CodeInput}
for i, j, sim in topica.align_topics(lda, bert):
    print("LDA t%-2d [%s] ~ BERTopic t%d [%s]  cos=%.2f"
          % (i, ", ".join(w for w, _ in lda.top_words(3)[i]),
             j, ", ".join(w for w, _ in bert.top_words(3)[j]), sim))
\end{CodeInput}
\begin{CodeOutput}
LDA t1  [peopl, will, american] ~ BERTopic t1 [obama, will, one]  cos=0.43
LDA t4  [palin, polit, one] ~ BERTopic t2 [palin, sarah, media]  cos=0.30
LDA t6  [mccain, john, campaign] ~ BERTopic t0 [mccain, campaign, obama]  cos=0.18
\end{CodeOutput}
\end{CodeChunk}

The two families, fit on the same corpus, agree on the themes a reader would
recognize: both isolate a Sarah Palin topic and a John McCain topic, even though
one learns from word counts and the other from document embeddings. Where they
diverge is granularity, not substance: the fifteen-topic \code{LDA} splits
material the three-cluster embedding model keeps whole. Answering the
introduction's question took one function, not a hand-built crosswalk between two
tools' output formats.

\subsection{One model in depth: estimating covariate effects}
...
% --- generated by paper/computational_details.py; do not hand-edit ---
The results in this article were produced with \pkg{topica}~0.11.0 on
\proglang{Python}~3.13.1, with \pkg{NumPy}~2.4.6, \pkg{SciPy}~1.17.1,
\pkg{scikit-learn}~1.9.0, \pkg{Matplotlib}~3.10.9, and \pkg{Plotly}~6.8.0. The
\proglang{Rust} core was compiled with \code{rustc}~1.85.0. The
cross-implementation checks of Section~\ref{sec:validation} and the timings of
Section~\ref{sec:performance} ran the reference packages \proglang{Java}~15.0.1
\pkg{MALLET}~2.0.8 (Homebrew build 202108) and \proglang{R}~4.5.2 with
\pkg{stm}~1.3.8 and \pkg{keyATM}~0.5.5. All computations were performed on
macOS~26.5 (Apple Silicon), \code{arm64}. This block is regenerated from the live
toolchain by \code{paper/computational\_details.py}.
