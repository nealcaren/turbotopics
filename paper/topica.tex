...
%% TODO(figure): generate paper/fig_poliblog_effect.pdf with
%% topica.viz.effect_plot(model, corpus, X=X) on the real poliblog corpus and
%% insert here. Caption: per-topic difference in prevalence between conservative
%% and liberal blogs, 95% method-of-composition intervals.

The fit recovers the substantive pattern the \pkg{stm} vignette documents: topics
that distinguish the two sides of the campaign load differently by blog rating,
and the differences carry method-of-composition intervals that separate them from
zero. The full, runnable version of this example, with the figure, is in the
