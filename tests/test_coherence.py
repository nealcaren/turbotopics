...
class TestCoherenceRanksTopics:
    ...
    def test_coherent_beats_incoherent(self, reference, ct):
        s = tt.coherence([COHERENT, INCOHERENT], reference, coherence_type=ct, topn=3)
        assert s.shape == (2,)
        assert s[0] > s[1], f"{ct}: coherent {s[0]} !> incoherent {s[1]}"

    def test_per_topic_shape(self, reference):
        s = tt.coherence([COHERENT, INCOHERENT, ["a", "b", "c"]], reference, coherence_type="c_v", topn=3)
        assert s.shape == (3,)


class TestRanges:
    def test_npmi_in_unit_range(self, reference):
        s = tt.coherence([COHERENT, INCOHERENT], reference, coherence_type="c_npmi", topn=3)
    ...
    def test_cv_nonnegative_ish(self, reference):
        ...
        s = tt.coherence([COHERENT, INCOHERENT], reference, coherence_type="c_v", topn=3)
        assert np.all(s >= -0.01) and np.all(s <= 1.0001)

    def test_umass_nonpositive_for_rare(self, reference):
        s = tt.coherence([INCOHERENT], reference, coherence_type="u_mass", topn=3)
        assert s[0] <= 0.0
...
class TestApi:
    def test_invalid_type_raises(self, reference):
        with pytest.raises(ValueError):
            tt.coherence([COHERENT], reference, coherence_type="c_bogus")
    ...
    def test_accepts_fitted_model(self, reference):
        ...
        s = tt.coherence(m, docs, coherence_type="c_npmi", topn=3)
        assert s.shape == (2,)
        assert np.all(np.isfinite(s))

    def test_accepts_word_prob_pairs(self, reference):
        topics = [[("a", 0.5), ("b", 0.3), ("c", 0.2)]]
        s = tt.coherence(topics, reference, coherence_type="c_v", topn=3)
        assert s.shape == (1,)

    def test_window_size_override(self, reference):
        s = tt.coherence([COHERENT], reference, coherence_type="c_npmi", topn=3, window_size=5)
        assert s.shape == (1,) and np.isfinite(s[0])

    def test_default_is_cv(self, reference):
        a = tt.coherence([COHERENT], reference, topn=3)
        b = tt.coherence([COHERENT], reference, coherence_type="c_v", topn=3)
        assert np.allclose(a, b)


class TestDiversity:
    def test_disjoint_is_one(self):
        assert tt.topic_diversity([["a", "b", "c"], ["d", "e", "f"]], topn=3) == 1.0

    def test_identical_is_half(self):
        # two identical 3-word topics: 3 unique / 6 total.
        assert tt.topic_diversity([["a", "b", "c"], ["a", "b", "c"]], topn=3) == 0.5

    def test_accepts_model(self):
        docs = [["cat", "dog", "pet"]] * 20 + [["star", "moon", "sky"]] * 20
        m = LDA(num_topics=2, seed=1)
        m.fit(docs, iterations=300)
        d = tt.topic_diversity(m, topn=3)
        assert 0.0 < d <= 1.0
